...
end_page = 133 # 할당받은 페이지로 수정하세요.



for i in range(10, end_page+1): #할당받은 끝 페이지
    ...
    try:
        ...
        for j in range(1, 26): #25 -- 한 페이지에 25개 소설
            ...
            # if driver.find_element('xpath', '//*[@id="content"]/div/ul/li[{}]/div/h3/em'.format(j)) == True:
            #     print('audult')
            # else:
            ...
            try:
                ...
                try:
                    for k in range(1, comment_range):
                        comment_page = [1, 3, 4, 5, 6]
                        try:
                            for l in range(1, 16):
                                comment_xpath = '//*[@id="cbox_module_wai_u_cbox_content_wrap_tabpanel"]/ul/li[{}]/div[1]/div/div[2]'.format(l)

                                try:
                                    comment = driver.find_element('xpath', comment_xpath).text
                                    comments.append(comment)
                                except:
                                    print('comment_each', i, j, k, l)
                            try:
                                for m in comment_page:
                                    driver.find_element('xpath', '//*[@id="cbox_module"]/div/div[7]/div/a[{}]'.format(m)).click()
                                    time.sleep(0.3)
                            except:
                                print('comment pages', i, j, k, l, m )
                        except:
                            print('comments', i, j, k)
                    driver.back()
                except:
                    print('error')
                # driver.back()
        ...
        df.to_csv('./crawling_data/naver_comments_{}_{}page.csv'.format(end_page, i), index=False)
    except:
        print('page', i)
