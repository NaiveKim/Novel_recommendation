...
form_window = uic.loadUiType('./naverseries.ui')[0]

class Exam(QWidget, form_window):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.Tfidf_matrix = mmread('./models/Tfidf_novel_review.mtx').tocsr()
        with open('./models/tfidf.pickle', 'rb') as f:
            self.Tfidf = pickle.load(f)
        self.embedding_model = Word2Vec.load('./models/word2vec_comments.model')
        self.comboBox.addItem('소설 리스트')
        self.df_reviews = pd.read_csv('./cleaned_comment_1480_one.csv')
        self.titles = sorted(set(list(self.df_reviews['cleaned_titles'])))
        self.titles.sort()
    ...
    def recommendation_by_movie_title(self, title):
        movie_idx = self.df_reviews[self.df_reviews['cleaned_titles'] == title].index[0]
